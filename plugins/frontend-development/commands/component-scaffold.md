  ...
  generateComponentBody(
    ...
    if (spec.state) {
      body += spec.state
        .map(
          (s) =>
            `  const [${s.name}, set${this.capitalize(s.name)}] = useState${
              options.typescript ? `<${s.type}>` : ""
            }(${s.initial});\n`
        )
        .join("");
      body += "\n";
    ...
    if (options.accessibility) {
      ...
      body += `    label: ${
        spec.props.find((p) => p.name === "label")?.name || `'${spec.name}'`
      }\n`;
      body += `  });\n\n`;
    }
...
${spec.props
  .map(
    (p) => `  ${p.name}${p.required ? "" : "?"}: ${this.mapNativeType(p.type)};`
  )
  .join("\n")}
}
...
${spec.props
  ...
  it('renders without crashing', () => {
    ...
    expect(screen.getByRole('${this.inferAriaRole(
      spec.type
    )}')).toBeInTheDocument();
  });
...
${spec.props
  ...
  it('calls ${p.name} when triggered', () => {
    ...
    render(<${spec.name} {...defaultProps} ${p.name}={mock${this.capitalize(
      p.name
    )}} />);
...
${spec.props
  .map(
    (p) =>
      `    ${p.name}: { control: '${this.inferControl(
        p.type
...
export const Default: Story = {
  args: {
${spec.props
  .map((p) => `    ${p.name}: ${p.defaultValue || this.getMockValue(p.type)},`)
  .join("\n")}
  },
};
